...
class student_registration_model extends CI_Model {
	...
	//	echo print_r($this->session->all_userdata()); exit; 
		$reg_from = "course";
		 $Query = "INSERT INTO eb_student_profile 
			...
			accomodation_info, email, password, register_from, house_name, house_no, street, city, state, post_code) VALUES 
			...
			'".$this->session->userdata('city')."', '".$this->session->userdata('state')."', '".$this->session->userdata('post_code')."')";
    ...
    {		
	//	echo print_r($this->session->all_userdata()); exit; 
		$chk_query = $this->db->query("SELECT email FROM eb_student_profile WHERE email = '".$this->session->userdata('email')."'");
		$dataResult = $chk_query->row();
		if($query->num_rows() > 0){
	//		echo "test"; exit;
			return "2";
		}else{

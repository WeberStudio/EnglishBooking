...
if ( ! function_exists('translate'))
{
	function translate($text, $d = '')
	{
		...
		echo $url = "https://www.googleapis.com/language/translate/v2?key=AIzaSyBSyBvMekGtewzkq0YkdK1DtPnv879TSjY&q=".$q."&source=".$s."&target=".$d;
		// Make sure to set CURLOPT_REFERER because Google doesn't like if you leave the referrer out
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_REFERER, "http://192.168.1.100/jamshaid/CodeIgniter/translate.php");
		$body = curl_exec($ch);
		curl_close($ch);
